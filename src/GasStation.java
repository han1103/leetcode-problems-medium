...
public class GasStation {
	public static void main(String[] args) {
		System.out
				.println(new GasStation().canCompleteCircuit(new int[] { 1, 2, 3, 4, 5 }, new int[] { 3, 4, 5, 1, 2 }));
		System.out.println(
				new GasStation().canCompleteCircuitFast(new int[] { 1, 2, 3, 4, 5 }, new int[] { 3, 4, 5, 1, 2 }));
		System.out.println(
				new GasStation().canCompleteCircuitSelfFast(new int[] { 1, 2, 3, 4, 5 }, new int[] { 3, 4, 5, 1, 2 }));
		System.out.println(new GasStation().canCompleteCircuitSelfThinking(new int[] { 1, 2, 3, 4, 5 },
				new int[] { 3, 4, 5, 1, 2 }));
	}
	...
	public int canCompleteCircuitSelfThinking(int[] gas, int[] cost) {
		int accumulatedTotal = 0;
		int accumuFromIndex = 0;
		int index = -1;
		for (int i = 0; i < gas.length; i++) {
			accumulatedTotal += (gas[i] - cost[i]);
			accumuFromIndex += (gas[i] - cost[i]);
			if (accumuFromIndex < 0) {
				if (index != -1)
					index = -1;
				accumuFromIndex = 0;
			}
		...
		return accumulatedTotal >= 0 && index != -1 ? index : -1;
	}
